...
def goodmatches(matches, threshold, img1, img2, kp1, kp2, fname1, fname2):
    ...
    print "Anzahl good matches mit einem threshold von " + str(threshold) + " : " + str(len(good))
...
def match_and_draw(img_pairs):
    for pathes in img_pairs:
        ...
        descriptors1 = np.loadtxt(filename1 + "dp1.txt", dtype=float,delimiter=',')
        descriptors2 = np.loadtxt(filename2 + "dp2.txt", dtype=float,delimiter=',')
        detected_keypoints1 = np.loadtxt(filename1 + "kp1.txt",dtype=float,delimiter=',')
        detected_keypoints2 = np.loadtxt(filename2 + "kp2.txt",dtype=float,delimiter=',')
        print "Anzahl Keypoints " + filename1 + " : " + str(len(detected_keypoints1))
        print "Anzahl Keypoints " + filename2 + " : " + str(len(detected_keypoints2))
        ...
        pts1 = pts1[mask.ravel() == 1]
        pts2 = pts2[mask.ravel() == 1]
        np.savetxt(filename1+"FMatrix.txt",F,delimiter=',')
        np.savetxt(filename1+"Points1.txt",pts1,delimiter=',')
        np.savetxt(filename1+"Points2.txt",pts2,delimiter=',')
        lines = cv2.computeCorrespondEpilines(pts1.reshape(-1,1,2), 1, F)
        lines = lines.reshape(-1, 3)
